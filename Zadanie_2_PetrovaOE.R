# Петрова Ольга – создайте модель множественной линейной регрессии потоков 
# дневных потоков паров воды за период 2013 года по данным измерений 
...
#Отфильтруем по заданию данные только за весь период 2013 года
...
mod2 = lm(h2o_flux~ DOY + Tau + qc_Tau + rand_err_Tau + H + qc_H + rand_err_H + LE + qc_LE + qc_h2o_flux + rand_err_h2o_flux + h2o_flux
...
mod3 = lm(h2o_flux~ DOY + Tau + qc_Tau + qc_H + rand_err_H + qc_LE + rand_err_h2o_flux + h2o_flux
