...
#Создадим таблицу, содержащую координаты Самары
Samara = data.frame(id = "Samara", latitude = 53.2,  longitude = 50.15)
#Получим список всех станций вблизи Самары
Samara_around = meteo_nearby_stations(lat_lon_df = Samara, station_data = station_data,
                                         limit = 19, var = c("TAVG"),
                                         year_min = 1990, year_max = 2005)
#Samara_around это список единственным элементом которого является таблица, содержащая идентификаторы метеостанций отсортированных по их удалленности от Samara

#Получим данные с 1,4 и 6 метеостанций
#Создадим пустую таблицу куда запишем данные с метеостанций
all_Samara_data = data.frame()
...
for(i in c(1,4,6)){
  ...
  temp = meteo_tidy_ghcnd(stationid = Samara_around[["Samara"]][["id"]][i], date_min="1990-04-1", date_max="2005-08-31")
  #Оставим нужные столбцы
  temp = select(temp, id, date, tavg)
  #Присоединим полученные данные в результирующую таблицу с данными
  all_Samara_data = rbind(all_Samara_data, temp)
}

#преобразуем дату в месяц и день и добавим столбики
all_Samara_data = mutate(all_Samara_data, month = month(date), day = day(date))
all_Samara_data_without0 = all_Samara_data #сохраним резервную таблицу без обнуления

#обнулим значение температуры в невегетативный период
all_Samara_data[(all_Samara_data$month == 4 & all_Samara_data$day <= 14),"tavg"] = 0
all_Samara_data[(all_Samara_data$month == 8 & all_Samara_data$day >= 16),"tavg"] = 0

#Сгруппируем по месяцам
all_Samara_data = all_Samara_data %>% group_by(month)
all_Samara_data_without0 = all_Samara_data_without0 %>% group_by(month)
